                ...
                if (dialect == "Sorani" and script == "Arabic") or (
                    ...
                    for test_case in self.test_cases["mwe_tokenize"][dialect][script]:
                        for case in test_case["cases"]:
                            ...
                            self.assertEqual(
                                tokenizer.mwe_tokenize(case, separator=test_case["parameters"]["separator"],
            ...
            for script in self.options["scripts"]:
                if (dialect == "Sorani" and script == "Arabic") or (
                    ...
                    for case in self.test_cases["sent_tokenize"][dialect][script]:
                        tokenizer = Tokenize(dialect, script)
                        # print(case)
                        self.assertEqual(tokenizer.sent_tokenize(case),
                                         self.test_cases["sent_tokenize"][dialect][script][case])
