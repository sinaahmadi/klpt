...
"""Preprocessing module of the Kurdish Language Processing Toolkit (KLPT).
    ...
    Normalizing scripts and orthographies by using writing
    conventions based on dialects and scripts.
    The goal is not to correct the orthography but to normalize
    the text in terms of the encoding and common writing rules.
...
class Preprocess:
    ...
    def __init__(self, dialect, script, numeral="Latin"):
        ...
        Args:
            obj: an object of the Configuration class containing the followings:
            ...
            - preprocess_map (dict): a dictionary exported from a JSON file
                containing the mapping rules

        """
        with open("data/preprocess_map.json") as preprocess_file:
    ...
    def standardize(self, text):
        ...
        Open issues:
            ...
            - "pirtükê": "pirtûkê"?
            - Should `ı (LATIN SMALL LETTER DOTLESS I) <https://www.compart.com/en/unicode/U+0131>`_ be replaced by i
    ...
    def normalize(self, text):
        ...
        - Sorani-Arabic:
            ...
            - replace "ه" followed by zero-width non-joiner (ZWNJ, U+200C) with "ە" where ZWNJ is removed ("ره‌زبه‌ر" is converted to "رەزبەر"). ZWNJ in HTML (&zwnj;) is also taken into account.
